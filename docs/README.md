...
## Running the docs CI checks locally

Pull request CI runs the docs checks as the `Docs check (Mintlify)` job, driven by [praktika](../ci/README.md).
The job validates `docs.json` and page frontmatter (`mint validate`) and checks internal links and anchors, redirects, and external links (via `lychee`).
When a PR touches the translated trees or the quickstarts, additional locale and quickstarts checks run as well.

You can run the same job locally before pushing.

### Prerequisites

- A checkout that includes both the `docs/` and `ci/` folders (the sparse clone described above is enough).
- Python 3 — praktika needs only the standard library.
- Docker installed and running. The job executes inside the `clickhouse/docs-builder` image, which is pulled automatically with the right architecture tag for your machine. The image bundles `mint` and `lychee`, so nothing else needs to be installed on the host.
- Give Docker plenty of memory: `mint validate` parses the whole site and is run with an 8 GB Node heap.
...
To run only a single check, pass `--test` with a case-insensitive substring of the check name:

| `--test` flag | What it checks | Command |
|---------------|----------------|---------|
| `validate` | `docs.json` navigation, and that every page's MDX and frontmatter parse (`mint validate`) | `python3 -m ci.praktika run "Docs check (Mintlify)" --test validate` |
| `internal links` | Internal links and anchors resolve (`lychee`) | `python3 -m ci.praktika run "Docs check (Mintlify)" --test "internal links"` |
| `redirects` | Redirect rules point at pages that exist | `python3 -m ci.praktika run "Docs check (Mintlify)" --test redirects` |
| `external` | External links are reachable (non-blocking, reported as warnings) | `python3 -m ci.praktika run "Docs check (Mintlify)" --test external` |
| `locale links` | Links in the translated locale trees resolve | `python3 -m ci.praktika run "Docs check (Mintlify)" --test "locale links"` |
| `locale component` | Links in the localized snippet components resolve | `python3 -m ci.praktika run "Docs check (Mintlify)" --test "locale component"` |
| `quickstarts` | Quickstart frontmatter tags are valid and the generated explorer data is up to date | `python3 -m ci.praktika run "Docs check (Mintlify)" --test quickstarts` |

In CI the locale and quickstarts checks run only when a PR changes the relevant files; in a local run the changed-file list is unavailable, so they always run — use `--test` to run only the check you need.

Notes:

- If you have `mint` and `lychee` installed on the host, you can add `--no-docker` to run without Docker.
