...
You can set the file name containing the format schema with setting `format_schema`.

The format schema is a combination of a file name and the name of the message type in this file, delimited by a colon.
For example: `schemafile.proto:MessageType`.

If the file has the standard extension for the format, it can be omitted. 
In this case, the format schema looks like `schemafile:MessageType`.
